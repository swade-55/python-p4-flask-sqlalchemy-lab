...
def zookeeper_by_id(id):
    ...
    if not animals:
        ...
        for animal in animals:
            response_body += f'<ul>Has animal {animal.species} named {animal.name}.</ul>'
    response = make_response(response_body,200)
    return response
...
def enclosure_by_id(id):
    ...
    if not enclosure:
        ...
        response = make_response(response_body,404)
        return response
    response_body = f'<h1>Information for {enclosure.environment}.</h1>'
